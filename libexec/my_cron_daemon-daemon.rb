...
DaemonKit::Cron.scheduler.every("5s") do
  DaemonKit.logger.debug "Scheduled task running in 5 sec completed at #{Time.now}"
end
DaemonKit::Cron.scheduler.every("15s") do
  DaemonKit.logger.debug "Scheduled task running in 15 sec completed at #{Time.now}"
end
